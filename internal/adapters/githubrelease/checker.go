// 文件职责：读取 GitHub Release 或兼容 manifest，选出可安装资产并解析 SHA256 校验信息。
...
const (
	// StatusNoUpdate 表示当前版本不低于最新正式 Release。
	StatusNoUpdate = "no_update"
	// StatusUpdateAvailable 表示找到更高版本、匹配安装资产和 SHA256 信息。
	StatusUpdateAvailable = "update_available"
	// StatusIgnored 表示本次检查被可恢复条件跳过，例如离线或 GitHub rate limit。
	StatusIgnored = "ignored"
	// StatusError 表示配置、HTTP、解析、资产或 SHA256 信息不可用。
	StatusError = "error"

	// SkipReasonOffline 表示网络离线导致本次检查或 .sha256 读取跳过。
	SkipReasonOffline = "offline"
	// SkipReasonRateLimit 表示 GitHub API 速率限制导致本次检查跳过。
	SkipReasonRateLimit = "rate_limited"
	// Sha256SourceDigest 表示 SHA256 来自 GitHub asset digest 字段。
	Sha256SourceDigest = "github_digest"
	// Sha256SourceAsset 表示 SHA256 来自同 Release 中匹配的 .sha256 资产。
	Sha256SourceAsset = "sha256_asset"
)

// AssetNamePolicy 返回指定版本允许匹配的安装资产名，顺序代表优先级。
type AssetNamePolicy func(version string) []string

// Config 是 Release 检查器配置。
type Config struct {
	// Owner 是 GitHub 仓库所有者；ManifestURL 非空时不使用。
	Owner string
	// Repo 是 GitHub 仓库名称；ManifestURL 非空时不使用。
	Repo string
	// CurrentVersion 是当前应用版本，用于 semver 比较。
	CurrentVersion string
	// APIBase 是 GitHub API 根地址；为空时使用官方 API。
	APIBase string
	// 本地静态 manifest 地址；非空时直接读取该 GitHub 兼容 JSON 数组。
	ManifestURL string
	// 更新源标识，用于向前端和日志说明本次检查来自 github 还是 local。
	Source string
	// ProxyBase 会作为 GitHub API、安装资产和 .sha256 下载 URL 的统一前缀。
	ProxyBase string
	// UserAgent 是所有 HTTP 请求使用的 User-Agent。
	UserAgent string
	// GitHub API 版本头。
	APIVersion string
...
// Checker 是无状态版本检查器；每次 Check 返回完整结果，不持久化历史。
type Checker struct {
	config Config
	client *http.Client
}
...
// githubRelease 是 GitHub Release API 和本地 manifest 共用的兼容结构。
...
// githubAsset 是 Release 资产的最小字段集。
...
// NewChecker 创建版本检查器并补齐默认 API 地址、source、请求头和 HTTP 客户端。
...
// Check 执行一次更新检查。
// ManifestURL 非空时读取本地静态 manifest；否则访问 GitHub Release API。
...
// CheckStatic 从内存 JSON 检查版本；不会联网解析 .sha256 资产。
...
// manifest 只改变数据来源，后续版本选择、资产匹配和 SHA256 解析仍复用 GitHub Release 规则。
...
// checkReleases 从 Release 列表中选最新正式版本、匹配安装资产，并填充可下载更新结果。
...
// fetchSha256Asset 下载并解析同 Release 中的 .sha256 文件。
...
// releaseAPIURL 返回 GitHub Release 列表 API URL。
...
// ignoredOfflineResult 返回可恢复的离线跳过结果，而不是 error。
...
// selectLatestRelease 选择版本号最高的正式 Release，跳过 draft、prerelease 和非 semver tag。
...
// selectAsset 按调用方提供的资产名优先级匹配安装资产，并查找同名 .sha256 辅助资产。
...
// findAsset 按文件名大小写不敏感匹配 Release 资产。
...
// sha256Pattern 用于从 .sha256 文本中提取第一个 SHA256。
...
// parseDigestSha256 只接受完整的 "sha256:<64 hex>" digest，拒绝带尾随文本的值。
...
// ParseSha256Text 从 .sha256 文件文本中提取 SHA256。
...
// proxiedURL 为 GitHub 相关 URL 添加代理前缀；已带同一前缀时保持幂等。

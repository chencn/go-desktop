...
授权码模式默认关闭。没有 `.env` 或没有配置 `GO_DESKTOP_LICENSE_MODE` 时，本地开发和打包产物都不要求授权。
...
GO_DESKTOP_LICENSE_PUBLIC_KEY=<base64url 公钥>
```

`GO_DESKTOP_LICENSE_PUBLIC_KEY` 会被打包进客户端，只用于验证授权码。`GO_DESKTOP_LICENSE_PRIVATE_KEY` 可以写在本地 `.env` 里给签发脚本读取，但真实 `.env` 不能提交，私钥不能进入源码、GitHub、安装包或 SQLite。

本地运行或打包授权版必须通过 `scripts/envrun` 启动 Wails task，让 `.env` 在 Taskfile `{{env ...}}` 展开前生效：
...
```powershell
go run ./scripts/license_keygen.go
```

脚本输出：

```text
publicKey=<base64url 公钥>
privateKey=<base64url 私钥>
```
...
```powershell
go run ./scripts/license_issue.go -device GD-7K3F-9P2X-MQ8C
```

`.env` 中的私钥配置：

```env
GO_DESKTOP_LICENSE_PRIVATE_KEY=<base64url 私钥>
```

私钥只给本地签发脚本读取，不会通过当前构建参数打包进客户端；不要提交真实 `.env`，也不要放到 GitHub、安装包或 SQLite。

授权码是 `GD1-...` 字符串，用户复制到授权页。客户端启动时会从 SQLite `config_items` 读取 `license.key`，再用内置公钥重新验签；不能只依赖数据库里的“已授权”标记放行。

GitHub Release 不读取 `.env`。官方授权发行版通过 GitHub Repository Variable 配置公钥：

```text
Settings -> Secrets and variables -> Actions -> Variables
Name: GO_DESKTOP_LICENSE_PUBLIC_KEY
Value: <base64url 公钥>
```
...
不要在 GitHub 配置 `GO_DESKTOP_LICENSE_PRIVATE_KEY`。
